...
public class ScheduleMatching {
      ...
      for (int shiftNumber = 0;
        ...
        for (int day = 0; day < days.length; day++) {
          if (threadsController.getSpreadsheetReader().getIsEmployeeAvailableOnDay(employee, day)) {
            int weight =
                threadsController.getSpreadsheetReader().isFreeDay(day)
                    ? weightForFreeDay
                    : Config.WEIGHT_FOR_NORMAL_DAY;
            weight =
                threadsController.getSpreadsheetReader().getEmployeeOnFixedDay(day) == employee
  ...
  private Solution getSolutionFromMatching(Set<DefaultWeightedEdge> matchingResult) {
    ...
    return new Solution(
        solution, numberOfFreeDaysForEmployee, threadsController.getSpreadsheetReader());
  }
